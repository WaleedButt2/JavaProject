...
public class Add_Friends extends HttpServlet {
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //
        if(nc.searchFriend((String)session.getAttribute("email"),Email).compareTo("")!=0){
            out.println("Your are already Friends with this person<br>");
            out.println("<form action=\"./Manage_Friends\" method=\"post\">");
            out.println("<input type=\"submit\" value=\"<--Go Back\"></body></html>");
            return;
        }
        nc.InsertFriends(new friends((String)session.getAttribute("email"),Email,Name));
        out.println("You are now Friends<br>May You not have any more<br>");
        out.println("<form action=\"./Manage_Friends\" method=\"post\">");
        out.println("<input type=\"submit\" value=\"<--Go Back\">");
        out.println("</body></html>");
    }
